<?php
  ...
  function datasSelectBdd($mail, $pwd) {
    ...
    // requête de selection des champs 'password' et 'email' dans la table 'users_connect' qui ont pour valeur les paramètres de la fonction.
    // c'est à dire que l'on selectionne des valeurs précises dans la bdd. 
    $dataSelectFieldsBdd = "SELECT email , password FROM user_connect WHERE password='$pwd' AND email='$mail';";
